...
Trade Size (Counter-intuitive): Traders use larger position sizes during Fear ($7,182) vs. Greed ($4,574). This may reflect experienced traders "buying the fear dip" with conviction, or survivorship bias (only large traders remain active in volatile Fear environments).
Long Bias Drops in Fear: The L/S ratio drops from 3.64 (Greed) to 3.02 (Fear), meaning traders add more short exposure during Fear days — a rational hedging behavior.
Trade Frequency is Stable: Daily trade counts are nearly equal across sentiment regimes (~340–350/day), indicating traders do not stop trading during Fear — they simply adjust sizing and direction rather than volume.
...
---
Methodology Summary
Data Join: Inner join on normalized trade date (UTC → IST date extraction from `Timestamp IST`)
PnL Analysis: Restricted to closed trades only; open trades excluded (PnL = 0)
Leverage Proxy: Relative position sizing (`Size USD / per-account median`) used since raw leverage is not in the dataset
Segmentation: Rule-based (quantile splits) + KMeans (k=4, StandardScaler, PCA for visualization)
Statistical Testing: Independent samples t-test (Fear vs. Greed PnL); effect confirmed at p < 0.0001
Visualization: 10 charts covering performance, behavior, segmentation, time-series, and clustering
---
